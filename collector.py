...
Fetches the configured news writer/tag pages + DuckDuckGo searches, filters by
keywords, dedupes against the existing data.json, and rewrites data.json.

No Claude, no browser. X/Twitter is NOT collected here (needs the paid X API);
add it later if you upgrade.
...
KEYWORDS = ["קשת 12","ערוץ 12","חדשות 12","ערוץ 14","כוח מאה","גיא פלג","דודי ורטהיים",
    "קוקה קולה","חרם ערוץ 12","שלמה קרעי","חוק התקשורת","החוק להחלשת התקשורת","הרשות השנייה",
...
# DuckDuckGo HTML searches. Sites that block direct fetch (Israel Hayom) + the
# Google-query topics from the spec. `unconditional`=True → keep every result
# (the spec: Google searches enter with no keyword condition).
SEARCHES = [
...
def scrape_search(s):
    url = "https://html.duckduckgo.com/html/?q=" + urllib.parse.quote(s["q"])
    html = get(url)
    if not html:
        return []
    soup = BeautifulSoup(html, "html.parser")
    ...
    for a in soup.select("a.result__a"):
        title = " ".join(a.get_text(" ", strip=True).split())
        href = a.get("href","")
        m = re.search(r"uddg=([^&]+)", href)      # DDG wraps target in a redirect
        if m:
        ...
        if not s["unconditional"] and not kws:
            continue
        out.append({"title":title,"url":href,"source":s["source"],
                    "type":"google","date":TODAY,"keywords":kws})
    return out[:8]                                  # cap per search
...
def main():
    ...
    merged.sort(key=lambda i: (str(i.get("date","")), ), reverse=True)
    merged = merged[:400]
